		 ...
		 fd = open(filename, O_RDWR|O_CREAT,0666);
		  ...
		  if(fd==-1){
			...
			if ((fd == -1) && (errno != EEXIST)){ 
				...
				//caso in cui ho creato il file nuovo
...
int DiskDriver_readBlock(DiskDriver* disk, void* dest, int block_num){
    ...
    if((arrayBitm[chiave.entry_num] >> chiave.bit_num & 1)==0){
		handle_error("blocco libero impossibile leggere");
	}
		
	//allineo il fd al blocco che devo andare a leggere (header + bitmapdim + 512*blocco da leggere
	int allineo = lseek(disk->fd, BLOCK_SIZE + disk->header->bitmap_blocks+ (BLOCK_SIZE*block_num) ,SEEK_SET);
	if(allineo ==-1){
		return -1;
	}
...
int DiskDriver_writeBlock(DiskDriver* disk, void* src, int block_num){
	...
	//finchè il mio blocco non è finito vado a leggere le info dal buffer src e le scrivo nel fd
		...
		if (val == -1){
			handle_error("errore di scrittura");
			}
...
int DiskDriver_freeBlock(DiskDriver* disk, int block_num){
	...
	int fd = disk->fd;
	int allineo = lseek(fd,BLOCK_SIZE + disk->header->bitmap_blocks+ (BLOCK_SIZE*block_num),SEEK_SET);
	if(allineo ==-1){
		return -1;
	}
